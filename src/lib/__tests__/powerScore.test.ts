...
describe("computePowerScore", () => {
  ...
  it("mythics score >= commons", () => {
    const commons = Array.from({ length: 20 }, () => card({ rarity: "common" }));
    const mythics = Array.from({ length: 20 }, () => card({ rarity: "mythic" }));
    expect(computePowerScore(mythics)).toBeGreaterThanOrEqual(computePowerScore(commons));
  });
